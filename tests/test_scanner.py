...
def test_discover_claude_finds_project_mcp(demo_project: Path):
    specs = discover_claude_code(demo_project)
    names = {s.name for s in specs}
    assert {"demo-tools", "search-service"} <= names


def test_live_scan_measures_tools_and_instructions(demo_project: Path):
    report = scan(client="claude", project_dir=demo_project)
...
def test_no_launch_gives_estimates(demo_project: Path):
    report = scan(client="claude", project_dir=demo_project, launch_servers=False)
